...
export class Translator {
  ...
  async translate (text, { from = 'en' } = {}) {
    ...
    // Single-flight the QVAC calls: overlapping translate() on the same model crosses the
    // worker's RPC responses (wrong text comes back). Chain every call so at most one runs
    // at a time. Sequential callers are unaffected; concurrent callers are made safe.
    this._chain = (this._chain || Promise.resolve()).then(async () => {
      if (this._cache.has(ck)) return
